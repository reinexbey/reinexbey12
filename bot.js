        ...
        try {
          ...
          msg.channel.sendEmbed(
            new Discord.RichEmbed()
              ...
              .setFooter(
                "Please select a figure between 1-10 and the list will be cancelled in 10 seconds."
              )
              .setColor("BLACK")
          );
          msg.delete(5000);
          ...
          try {
            ...
            return msg.channel.sendEmbed(
              new Discord.RichEmbed()
                .setColor("BLACK")
                .setDescription(
                  ":x: ***Selection cancelled for not specifying Song Value**."
                )
            );
          }
...
async function handleVideo(video, msg, voiceChannel, playlist = false) {
  ...
  if (!serverQueue) {
    ...
    return msg.channel.sendEmbed(
      new Discord.RichEmbed()
        .setTitle(
          `:arrow_heading_up:  **${song.title}** Named Music Added to The Queue!`
        )
        .setColor("BLACK")
    );
  }
...
function play(guild, song) {
  ...
  const dispatcher = serverQueue.connection
    ...
    .on("end", reason => {
      if (reason === " :x: **Broadcast flow rate not enough.**")
        console.log("Song İs End");
...
client.on("message", msg => {
  if (msg.content.toLowerCase() === "!!invite") {
    const eris = new Discord.RichEmbed()
      ...
      .setDescription("Sunucumuz : https://discord.gg/4A9tUwF");
...
client.on("guildCreate", guild => {
  ...
  const embed = new Discord.RichEmbed().setTitle(
    "Selamlar chat ben geldim sabahlara kadar kopmaya hazır mısınız?"
  );
  kanal.send(embed);
});
